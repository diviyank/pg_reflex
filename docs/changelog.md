...
- (SILENT) A caught deferred-flush failure never set `known_stale` and the next successful flush cleared `last_error`; the IMV is now marked stale with a `stale_reason` naming the failure and `reflex_reconcile`, and `last_error` survives while stale.
- (SILENT) `reflex_ivm_status()` reported the pre-wipe `reltuples`; an anomalous IMV is now counted exactly and the new `is_estimate` column says which count the caller got.
- (SILENT) An IMV whose partition source root is at the flush failure cap reported healthy; it is now `known_stale`, directly or transitively, with the reset-and-flush remedy in `stale_reason`, derived live so a reconcile cannot hide it.
- (SILENT, CREATE-TIME) `create_reflex_ivm` accepted an `ignore_sources` entry the query filters or inner-joins on — the field wipe's mechanism; it is now refused unless acknowledged.
- (SILENT) A slice emptied while an ignored source excluded it never healed; an ignored source that joins onto the partition key now queues the changed keys in `__reflex_heal_pending`, and the IMV reports `known_stale` with the `reflex_heal_ignored_sources` remedy until a sweep rebuilds exactly those partitions; the `SECURITY DEFINER` trigger needs no pg_reflex grant nor `USAGE` on `public`, cannot be called directly, evaluates no user-defined function and never fails or taxes the write, a `NULL`↔`''` change of a watched column is queued, a `TRUNCATE` (also `TRUNCATE ONLY` of an inheritance parent) marks the IMV stale keeping any earlier reason, a renamed mapped column or a key retyped to a user type is reported stale from the catalog, and a cancel during a heal is re-raised.
- `reflex_reconcile` of a partitioned IMV with a dependent raised a false "source table … was altered" alarm for its own relocation trigger toggle, and aborted under `alter_source_policy = 'error'`; the toggle is now exempt for that one statement, and a user `ALTER` of the same relation is still reported.
- `reflex_reconcile` printed an `already exists, skipping` NOTICE per partition child and sync pass; the sync now creates only missing children, and the deferred cross-source marker is created once per batch.
- `reflex_ivm_status()` counted an actively swapped partitioned IMV exactly on every call; the swap now retires its own `rebuild` event by stamping it before the leaf ANALYZE, and the estimate sums the leaves.
- An invalid `pg_reflex.flush_failure_policy` value now raises a `WARNING` naming it before falling back to `warn`.

**Added**

- `__reflex_event_log` (caught flush failures and slice-changing rebuilds) and `reflex_prune_event_log(INTERVAL)`.
- `pg_reflex.flush_failure_policy` GUC — `warn` (default) or `error` to abort the caller's transaction.
- `reflex_ack_ignore_source(imv, source)`, the create-time `'!source'` marker and the `ignore_ack` registry column.
- `ignore-soundness` audit check, forwarded by `reflex_doctor` as F13 (report-only).
- `reflex_heal_ignored_sources(imv)`, run first by `reflex_scheduled_reconcile` and by `reflex_doctor(fix => TRUE)` as F14.

**Changed**

- `create_reflex_ivm` — and therefore `reflex_rebuild_chain`, which replays it — refuses unsound `ignore_sources`; run `SELECT * FROM reflex_audit() WHERE category = 'ignore-soundness'` and acknowledge before recreating an IMV or rebuilding a chain. A client that drops and recreates IMVs must add the `'!'` marker to the entries it accepts. `reflex_rebuild_imv` (a reconcile) and `create_reflex_ivm_if_not_exists` of an existing IMV are not affected.
- `reflex_ivm_status()` returns one extra trailing column, `is_estimate`, and estimates a partitioned IMV from its leaves.
- `reflex_scheduled_reconcile` returns one `HEALED` row per healed IMV before its reconcile rows; clients must treat `HEALED` as success.

**Tests**

- New `pg_test_status_window_wipe.rs`, `pg_test_ignore_soundness.rs`, `pg_test_capped_source_status.rs`, `pg_test_ignored_source_heal.rs` and `pg_test_reconcile_log_noise.rs`, every must-refuse / must-report assertion mutation-checked.

**Migration**

- [`sql/pg_reflex--1.11.3--1.11.4.sql`](https://github.com/diviyank/pg_reflex/blob/main/sql/pg_reflex--1.11.3--1.11.4.sql) — adds `ignore_ack`, `__reflex_event_log`, `reflex_prune_event_log`, `__reflex_heal_pending`, its trigger function, recreates `reflex_ivm_status()` with `is_estimate` (DROP + CREATE), creates `reflex_ack_ignore_source` and `reflex_heal_ignored_sources`, replaces `__reflex_on_ddl_command_end`, and installs heal triggers on existing IMVs via `reflex_rebuild_imv_metadata`.

## [1.11.3] — 2026-07-28

Four correctness fixes on the partitioned-IMV maintenance paths, three of them silent — they produced wrong or absent data with no error, so an affected cluster shows no symptom until someone reads the result. Two destroyed data outright: a partition swap that flattened a sub-partitioned mirror child so the next sync emptied the IMV (2.8 M rows → 0 in the reproduction), and a `reflex_reconcile` that destroyed every dependent IMV of a partitioned one. The other two are a failure-atomicity fix that also closes a backend-abort class, and the removal of an `AccessExclusive` lock that froze every reader of an IMV — including readers of unrelated partitions — whenever a source partition was added. This migration **must** be applied: the dependent-cascade fix is split between the library and the `__reflex_on_ddl_command_end` trigger body, and the new library paired with the old trigger is worse than either half alone. `ALTER EXTENSION pg_reflex UPDATE TO '1.11.3';`

**Fixed**

- (DATA LOSS) The partition swap built its replacement with `CREATE TABLE ... (LIKE old INCLUDING ALL)`, and `LIKE` never copies partitioning, so a depth-≥2 mirror child became a plain table — data stayed correct at that instant, then the next sync's shape-drift heal dropped those children and recreated them EMPTY with NOTICE-level output only; `reflex_reconcile` now resolves mirror leaves and the swap refuses a partitioned child outright.
- An IMV already flattened in the field still holds correct data but is armed: repair it with `reflex_reconcile('<imv>')`, and do **not** run `reflex_sync_partitions` on it first — that is the step that empties it.
- (DATA LOSS) `reflex_reconcile` on a partitioned IMV destroyed its dependents, because each swap's `ALTER TABLE` fired the `ddl_command_end` trigger whose auto-sync re-mirrored the parent's transient mid-swap child set and dropped the real child as an orphan; the swap now publishes `pg_reflex.internal_swap_root` for the duration of its DETACH/ATTACH, the trigger returns while it is set, and dependents are refreshed explicitly afterwards — which also deletes an O(N²·D) term, making reconcile ~3.9× faster at 50 partitions.
- (SILENT PARTIAL COMMIT / BACKEND ABORT) `reflex_reconcile_partition` committed its pre-sync's `DROP TABLE ... CASCADE` even when it reported `ERROR`, and a multi-child reconcile failing on child N committed children 1..N-1's swaps; the pre-sync and reconcile body now share one subtransaction, on the batch flush path as well.
- (AVAILABILITY) Adding a source partition created the mirror child in place with `CREATE TABLE ... PARTITION OF <live IMV root>`, taking `AccessExclusive` on the root and holding it through the COMMIT-time reconcile; new nodes are now built detached, attached empty and filled in place, keeping the root at `ShareUpdateExclusive` at any depth — measured at depth 1, root `AccessExclusive` window 0.48 s → none and reader latency 184 ms → 13 ms.

**Tests**

- New `pg_test_subpartition_dataloss.rs`, `pg_test_reconcile_dependent_dataloss.rs` and `pg_test_partition_attach_locks.rs` (lock shape asserted from a second session), every assertion mutation-checked — reverting the guard it pins turns it RED.
- `benchmarks/bench_partition_scaling.sh`, a reusable partition-count scaling harness that reproduces the 1.11.1 flush regression it was built to guard against (53.2 ms vs 4.9 ms median at N=200).

**Migration**

- [`sql/pg_reflex--1.11.2--1.11.3.sql`](https://github.com/diviyank/pg_reflex/blob/main/sql/pg_reflex--1.11.2--1.11.3.sql) — replays the `__reflex_on_ddl_command_end` function body, which gained an early return while `pg_reflex.internal_swap_root` is set. Not optional: the new library against the old trigger body still corrupts dependent IMVs mid-swap. No registry columns added or changed.

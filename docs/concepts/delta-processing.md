...
|---|---|---|---|
| `INSERT` / `UPDATE` / `DELETE`, low-selectivity (default) | `MERGE` into intermediate + targeted `DELETE`+`INSERT` on target (`src/trigger/ops.rs`, `src/trigger/merge.rs`) | `RowExclusiveLock` | **No** — compatible with `AccessShareLock` |
...
| `UPDATE` on a self-join source (`is_self_join` branch, `src/trigger/ops.rs` · `self_join_full_refresh_stmts`) | `TRUNCATE` target + full `INSERT` from base query | `AccessExclusiveLock` | **Yes** |
| `TRUNCATE` on source | `DELETE FROM target` (+ `TRUNCATE` intermediate for aggregate IMVs) (`src/trigger/mod.rs` · `reflex_build_truncate_sql`) | `RowExclusiveLock` on target; `AccessExclusiveLock` on the *intermediate* (internal) | **No** on the target — concurrent readers continue against MVCC-visible rows until the deleting txn commits |
...
The dispatcher's selectivity check (`src/trigger/dispatch.rs` · `build_high_selectivity_dispatch_sql`, the `DO` block injected ahead of every grouped MERGE) reads `affected_groups / intermediate_size` and compares against the per-IMV `wipe_threshold` column, then the `reflex.wipe_threshold` GUC, then the compiled default. Below the threshold, the run takes the incremental branch (no reader block); at or above, it delegates to either an inline bulk path (Path B / Path C, 1.5.0+) or `reflex_reconcile`. Only the `reflex_reconcile` branch and the aggregated self-join branch take `AccessExclusiveLock` on the target. A source `TRUNCATE` locks only the intermediate at that level — the target sees a `DELETE`.

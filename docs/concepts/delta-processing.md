...
| `TRUNCATE` on source | `DELETE FROM target` (+ `TRUNCATE` intermediate for aggregate IMVs) (`trigger.rs:2299-2300`) | `RowExclusiveLock` on target; `AccessExclusiveLock` on the *intermediate* (internal) | **No** on the target — concurrent readers continue against MVCC-visible rows until the deleting txn commits |

...
- **Target-drop cleanup for the `sql_drop` event trigger (1.10.6).** Dropping an IMV's own target table — directly, via `DROP SCHEMA … CASCADE`, or by dropping a view an IMV is built on — no longer orphans its registry row. The trigger previously matched only dropped *source* tables; it now also tears down the IMV whose registered target *is* the dropped table.
- **Cheaper attach of partitions with many empty sub-partitions (1.10.5).** The batch flush syncs each partition tree once up front and skips brand-new leaves whose source is empty, instead of reconciling every leaf and re-syncing the whole tree per leaf — a 48-leaf attach with 44 empty months dropped 898s → 51s (~17.6×, debug build).

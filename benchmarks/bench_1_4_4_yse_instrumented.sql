-- ==========================================================================
-- Probe: instrumented UPDATE trigger on yse.demand_planning
--
-- Goal: attribute the ~4 s unattributed gap between per-statement
-- auto_explain (~12 s) and wall-clock UPDATE (~15.9 s) on the customer's
-- yse.ivm_sop_forecast_view IMV, post-not_null_columns-patch.
--
-- Adapted from benchmarks/bench_1_4_4_gap_A_instrumented.sql (rb shape).
--
-- Method: hot-swap public.__reflex_upd_trigger_on_yse_demand_planning() with
-- an instrumented body that logs clock_timestamp() deltas around every step
-- (exists check, ref-table SELECT, where_predicate check, advisory lock,
-- Rust FFI to reflex_build_delta_sql, every EXECUTE _stmt). Restore the
-- original body in a finally-style block.
--
-- Each measured run is wrapped in BEGIN/ROLLBACK so the IMV state is
-- preserved across iterations (no dead tuple accumulation).
--
-- Connection: psql -h <host> -U <user> -d db_dev -f <this>
-- ==========================================================================
\timing on
\set ON_ERROR_STOP off

-- =========================================================
-- 0. Sanity: confirm the not_null_columns patch is still in place
--    (must include dem_plan_id, product_id, location_id).
-- =========================================================
SELECT name,
       aggregations::jsonb->'not_null_columns' AS not_null_cols
FROM public.__reflex_ivm_reference
WHERE name = 'yse.ivm_sop_forecast_view';
...
WHERE p.proname='__reflex_upd_trigger_on_yse_demand_planning'
  AND n.nspname='public';

SELECT COUNT(*) AS saved FROM _saved_upd_trigger;
-- ^ must be 1. If 0, the trigger name doesn't match — stop here and look it up:
--    SELECT proname FROM pg_proc WHERE proname LIKE '\_\_reflex\_upd\_trigger\_on\_yse%' ESCAPE '\';
...
-- =========================================================
CREATE OR REPLACE FUNCTION public.__reflex_upd_trigger_on_yse_demand_planning()
...
BEGIN
    ...
    SELECT EXISTS(SELECT 1 FROM "__reflex_new_yse_demand_planning" LIMIT 1) INTO _has_rows;
    ...
    FOR _rec IN
        ...
        FROM public.__reflex_ivm_reference
        WHERE 'yse.demand_planning' = ANY(depends_on) AND enabled = TRUE
    ...
    LOOP
        ...
        IF _rec.where_predicate IS NOT NULL THEN
            ...
            EXECUTE format('SELECT EXISTS(SELECT 1 FROM %I WHERE %s LIMIT 1)',
                '__reflex_new_yse_demand_planning', _rec.where_predicate) INTO _pred_match;
            INSERT INTO _gap_timings(run_label, step_idx, step_name, stmt_excerpt, ms)
        ...
        _sql := public.reflex_build_delta_sql(
            _rec.name, 'yse.demand_planning', 'UPDATE',
...
-- =========================================================
-- 4. Warm-up run (rolled back, not counted)
-- =========================================================
...
-- =========================================================
-- 5. Three repeats — COMMITTED so the trigger's _gap_timings
--    inserts survive. The trigger fires perform a no-op status round-trip
--    (validated → validated is byte-equal, so 1.4.3's spurious-skip might
--    NOT fire here — the source-col projection includes status; we observe
--    via run_1's wall-clock whether it did). Each run accumulates ~555 K
--    dead tuples in intermediate + target. Run VACUUM after.
-- =========================================================
...
BEGIN
    ...
    IF _def IS NULL THEN
        RAISE WARNING 'No saved trigger body found — restore manually from a backup';
    ELSE
        EXECUTE _def;
    END IF;
END $$;
...
\echo ''
\echo '=== Per-statement summary (the EXECUTE _stmt branch) ==='
SELECT step_name,
       ROUND(AVG(ms)::numeric, 2) AS avg_ms,
       MAX(left(stmt_excerpt, 120)) AS stmt
FROM _gap_timings
